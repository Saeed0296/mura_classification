...
MODEL_ARCH = "densenet169"  # DenseNet169 is standard baseline for MURA
BATCH_SIZE = 64            # M4 with 24GB unified memory can handle batch size 64
...
def run_training(category):
    print(f"\n==========================================")
    print(f"STARTING DENSENET169 TRAINING: {category}")
    print(f"==========================================")
    ...
    model_save_path = os.path.join(CHECKPOINT_DIR, f"mura_densenet_{category.lower()}_best_model.pth")
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if val_kappa > best_kappa:
            ...
            print(f"--> Saved new best DenseNet169 checkpoint (Kappa: {best_kappa:.4f})")

    print(f"DenseNet169 Training for {category} completed. Best Val Kappa: {best_kappa:.4f} | Best Val Acc: {best_acc:.4f}")
...
if __name__ == "__main__":
    ...
    for category in JOINT_CATEGORIES:
        try:
            ...
            print(f"\n[CRITICAL ERROR] Failed to train DenseNet169 model for {category}. Skipping to next category.")
    ...
    stats_save_path = os.path.join(CHECKPOINT_DIR, "densenet_training_statistics.json")
    with open(stats_save_path, "w", encoding="utf-8") as f:
        json.dump(overall_stats, f, indent=4)
        
    total_duration = time.time() - start_all
    print(f"\n==========================================")
    print(f"ALL DENSENET169 JOINT MODELS TRAINED IN SERIES SUCCESSFULLY!")
